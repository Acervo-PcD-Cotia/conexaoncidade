...
export function CategorySection({ title, slug }: CategorySectionProps) {
  const { data: news, isLoading } = useNewsByCategory(slug, 6);

  if (isLoading) {
    return (
      <section className="container py-4">
        <div className="animate-pulse">
          <div className="mb-4 h-6 w-32 bg-muted rounded" />
          <div className="grid gap-3 lg:grid-cols-3">
            <div className="lg:col-span-2 aspect-[16/10] bg-muted rounded-lg" />
  ...
  return (
    <section className="container py-4">
      ...
      <div className="grid gap-3 lg:grid-cols-3">
          ...
          <Link to={`/noticia/${mainNews.slug}`} className="group block">
            <article className="news-card relative overflow-hidden rounded-lg">
              <div className="aspect-[16/10] overflow-hidden">
                <img
                  src={mainNews.featured_image_url || "https://images.unsplash.com/photo-1585829365295-ab7cd400c167?w=800&h=500&fit=crop"}
                  alt={mainNews.title}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 text-white">
                ...
                <h3 className="font-heading text-lg font-bold leading-tight line-clamp-2 md:text-xl">
                  {mainNews.title}
                </h3>
        ...
        {/* Side news - text only, dense list */}
        <div className="rounded-lg border border-border bg-card">
          ...
          <div className="divide-y divide-border">
            {sideNews.slice(0, 5).map((n) => (
              <Link
                key={n.id}
                to={`/noticia/${n.slug}`}
                className="group flex items-start gap-2 px-3 py-2.5 transition-colors hover:bg-muted/50"
              >
                <span
